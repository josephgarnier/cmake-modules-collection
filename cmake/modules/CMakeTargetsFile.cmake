...
function(_get_json_array output_list_var json_block json_path)
	...
	if("${json_path}" STREQUAL "")
		message(FATAL_ERROR "json_path argument is missing or empty!")
	endif()
	...
	set(elements_list "")
	string(JSON nb_elem LENGTH "${json_block}" ${json_path})
	if (nb_elem GREATER 0)
		math(EXPR last_index "${nb_elem} - 1")
		foreach(i RANGE 0 ${last_index})
			string(JSON elem GET "${json_block}" ${json_path} ${i})
...
function(_get_json_object output_map_var json_block json_path)
	...
	set(objects_map "")
	string(JSON nb_objects LENGTH "${json_block}" ${json_path})
	if(nb_objects GREATER 0)
		math(EXPR last_index "${nb_objects} - 1")
		foreach(i RANGE 0 ${last_index})
			string(JSON prop_key MEMBER "${json_block}" ${json_path} ${i})
			string(JSON prop_value GET "${json_block}" ${json_path} "${prop_key}")
...
function(_json_object_to_map output_map_var json_object)
	...
	if(nb_objects GREATER 0)
		math(EXPR last_index "${nb_objects} - 1")
		foreach(i RANGE 0 ${last_index})
			string(JSON prop_key MEMBER "${json_object}" ${json_path} ${i})
...
macro(_cmake_targets_file_is_loaded)
	...
	if((NOT DEFINED is_file_loaded) OR (NOT ${is_file_loaded}))
...
function(_assert_target_config_exists target_dir_path)
	...
	get_property(is_config_var_defined GLOBAL PROPERTY "TARGETS_CONFIG_${target_dir_path}" DEFINED)
	get_property(does_config_exist GLOBAL PROPERTY "TARGETS_CONFIG_${target_dir_path}" SET)
	if((NOT ${is_config_var_defined}) OR (NOT ${does_config_exist}))
