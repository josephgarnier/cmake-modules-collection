...
#------------------------------------------------------------------------------
# [Internal use only]
function(_has_json_property json_block json_path_list output_var)
  if(NOT ${ARGC} EQUAL 3)
...
#------------------------------------------------------------------------------
# [Internal use only]
function(_get_json_value json_block json_path_list json_type is_required output_var)
  if(NOT ${ARGC} EQUAL 5)
...
#------------------------------------------------------------------------------
# [Internal use only]
function(_get_json_array json_block json_path_list is_required output_list_var)
  if(NOT ${ARGC} EQUAL 4)
  ...
  _json_array_to_list("${json_array}" ${output_list_var})
  return(PROPAGATE "${output_list_var}")
endfunction()

#------------------------------------------------------------------------------
# [Internal use only]
function(_json_array_to_list json_array output_list_var)
  if(NOT ${ARGC} EQUAL 2)
    message(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}() requires exactly 2 arguments, got ${ARGC}!")
  endif()
...
#------------------------------------------------------------------------------
# [Internal use only]
function(_get_json_object json_block json_path_list is_required output_map_var)
  if(NOT ${ARGC} EQUAL 4)
    message(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}() requires exactly 4 arguments, got ${ARGC}!")
  endif()
  if("${json_block}" STREQUAL "")
  ...
  _json_object_to_map("${json_object}" ${output_map_var})
  return(PROPAGATE "${output_map_var}")
endfunction()

#------------------------------------------------------------------------------
# [Internal use only]
function(_json_object_to_map json_object output_map_var)
  if(NOT ${ARGC} EQUAL 2)
    message(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}() requires exactly 2 arguments, got ${ARGC}!")
  endif()
...
#------------------------------------------------------------------------------
# [Internal use only]
function(_serialize_list input_list_var output_var)
  if(NOT ${ARGC} EQUAL 2)
    message(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}() requires exactly 2 arguments, got ${ARGC}!")
  endif()
  if("${input_list_var}" STREQUAL "")
    message(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}() requires 'input_list_var' argument to be a non-empty string value!")
  endif()
  if("${output_var}" STREQUAL "")
    message(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}() requires 'output_var' argument to be a non-empty string value!")
  endif()

  list(TRANSFORM ${input_list_var} REPLACE "\\|" "\\\\|")
  list(JOIN ${input_list_var} "|" ${output_var})
  return(PROPAGATE "${output_var}")
endfunction()

#------------------------------------------------------------------------------
# [Internal use only]
function(_deserialize_list encoded_string output_list_var)
  if(NOT ${ARGC} EQUAL 2)
    message(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}() requires exactly 2 arguments, got ${ARGC}!")
  endif()
  if("${output_list_var}" STREQUAL "")
...
#------------------------------------------------------------------------------
# [Internal use only]
function(_is_serialized_list input_string output_var)
  if(NOT ${ARGC} EQUAL 2)
    message(FATAL_ERROR "${CMAKE_CURRENT_FUNCTION}() requires exactly 2 arguments, got ${ARGC}!")
  endif()
  if("${output_var}" STREQUAL "")
  ...
  list(LENGTH input_string nb_vals)
  if(${nb_vals} GREATER 1)
    # False if input_string is a cmake list
    set(${output_var} false)
  else()
    # False if input_string contains no pipe; true otherwise
